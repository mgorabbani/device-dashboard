import React from 'react'
import moment from 'moment';
import PropTypes from 'prop-types';

class Device extends React.Component {
    constructor(props) {
        super(props)
        this.toggleState = this.toggleState.bind(this)
        this.state = { loading: false }
    }
    toggleState() {
        this.setState({ loading: true })
        this.props.toggle(this.props.device).then(() => {
            this.setState({ loading: false })
        })
    }
    render() {
        const { device } = this.props;
        return (<div className="col-md-4 mb-4">
            <div className="device">

                <h5>{device.name}
                    {device.active ? <span>{activeIcon} Active</span> :
                        <span>{deactiveIcon} Deactivate</span>}
                </h5>
                <li>Unit: {device.unit}</li>
                <li>Value: {device.value.toFixed(2)}</li>
                <li>Time: {moment(device.timestamp).fromNow()}</li>
                <br />
                <button
                    onClick={this.toggleState}
                    className="btn btn-outline-dark"
                    disabled={this.state.loading}>
                    Toggle Status
                </button>
            </div>
        </div>)
    }
}

Device.propTypes = {
    device: PropTypes.object.isRequired
}

const deactiveIcon = <img style={{ height: 20 }} src="data:image/png;base64," />

const activeIcon = <img style={{ height: 20 }} src="data:image/png;base64," />

export default Device;